...
describe('Auth Utils', () => {
  test('generateApiKey creates valid key', () => {
    ...
    assert(key.startsWith('stringtok_'), 'Should have correct prefix');
    assertEqual(key.length, 74, 'Should have correct length');
  });

  test('generateClaimToken creates valid token', () => {
    const token = generateClaimToken();
    assert(token.startsWith('stringtok_claim_'), 'Should have correct prefix');
  });
  ...
  test('validateApiKey rejects invalid key', () => {
    ...
    assert(!validateApiKey('stringtok_short'), 'Should reject short key');
  });

  test('extractToken extracts from Bearer header', () => {
    const token = extractToken('Bearer stringtok_test123');
    assertEqual(token, 'stringtok_test123');
  });
...
describe('Config', () => {
  test('config loads without error', () => {
    ...
    assert(config.stringtok.tokenPrefix, 'Should have token prefix');
  });
});
